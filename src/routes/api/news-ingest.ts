  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const channel = String(body.channelUsername ?? CHANNEL).replace(/^@/, "");

        if (!Number.isSafeInteger(messageId) || messageId <= 0) {
          return Response.json({ error: "messageId must be a positive integer" }, { status: 400 });
        }
        if (channel.toLowerCase() !== CHANNEL.toLowerCase()) {
          return Response.json({ error: "Invalid channel" }, { status: 400 });
        }
        if (Number.isNaN(publishedAt.getTime())) {
          return Response.json({ error: "Invalid publishedAt" }, { status: 400 });
        }
        if (Date.now() - publishedAt.getTime() > DAY_MS || publishedAt.getTime() - Date.now() > 5 * 60_000) {
        ...
        await sql.query(
          `delete from telegram_posts where published_at < now() - interval '24 hours'`,
        );
